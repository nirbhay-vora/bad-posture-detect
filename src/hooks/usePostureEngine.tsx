...
export function usePostureEngine() {
  ...
  // Cooldown counter to avoid spamming notifications
  const cooldownRef = useRef(0)
  
  // Timestamp for MediaPipe (it needs to know time for video mode)
  const lastVideoTimeRef = useRef(-1)

  // React state — these changes cause the UI to re-render
    ...
    if (result.landmarks.length > 0 && baseline) {
      ...
      if (percentage > 20) {
        ...
        if (slouchBufferRef.current >= SLOUCH_THRESHOLD && cooldownRef.current === 0) {
          console.warn('🚨 BAD POSTURE: You have been slouching for 10+ seconds! Sit up straight!')
          setShowAlert(true)
        ...
        // Good posture — reset the slouch buffer
        slouchBufferRef.current = 0
